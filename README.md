...
Если сайт не отдаёт подходящий favicon, его можно вручную запросить через
[Icon Horse](https://icon.horse/). Bastle передаёт только hostname, делает этот
запрос исключительно по нажатию кнопки и сохраняет нормализованный PNG локально;
без сети провайдер не нужен.
